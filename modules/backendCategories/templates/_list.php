<div class="sf_admin_list">
  ...
  <?php else: ?>
    <table id="main_list" cellspacing="0">
      <thead>
        <tr>
          ...
          <?php include_partial('backendCategories/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <tr>
          <th colspan="8">
            <?php if ($pager->haveToPaginate()): ?>
              <?php include_partial('backendCategories/pagination', array('pager' => $pager)) ?>
            <?php endif; ?>
   ...
   <tbody>
        <?php foreach ($pager->getResults() as $i => $backendCategories): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr id="node-<?php echo $backendCategories['id'] ?>" class="sf_admin_row <?php echo $odd ?><?php
          // insert hierarchical info
          $node = $backendCategories->getNode();
          if ($node->isValidNode() && $node->hasParent())
          {
            echo " child-of-node-".$node->getParent()->getId();
          }
          ?>">
            <?php include_partial('backendCategories/list_td_batch_actions', array('backendCategories' => $backendCategories, 'helper' => $helper)) ?>
            <?php include_partial('backendCategories/list_td_tabular', array('backendCategories' => $backendCategories)) ?>
            <?php include_partial('backendCategories/list_td_actions', array('peanut_categories' => $backendCategories, 'helper' => $helper)) ?>
          </tr>
        <?php endforeach; ?>
     </tbody>
